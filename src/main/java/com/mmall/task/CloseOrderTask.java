...
//    @Scheduled(cron = "0 */1 * * * ?")
    ...
    public void closeOrderTaskV3() {
        ...
        if (setnxResult != null && setnxResult.intValue() == 1) {
            ...
            String lockValueStr = RedisShardedPoolUtil.get(Const.REDIS_LOCL.CLOSE_ORDER_TASK_LOCK);
            if (lockValueStr != null && System.currentTimeMillis() > Long.parseLong(lockValueStr)) {
                 ...
                 * */
                String getSetResult = RedisShardedPoolUtil.getSet(Const.REDIS_LOCL.CLOSE_ORDER_TASK_LOCK, String.valueOf(System.currentTimeMillis()) + lockTime);
                /*
                 * 再次用当前时间戳getset
                ...
                if (getSetResult == null || StringUtils.equals(lockValueStr, getSetResult)) {
                    ...
                    log.info("没有获取到分布式锁：{}", Const.REDIS_LOCL.CLOSE_ORDER_TASK_LOCK);
                }
                /*在set之前，该锁已经消失*/
            } else {
                log.info("没有获取到分布式锁：{}", Const.REDIS_LOCL.CLOSE_ORDER_TASK_LOCK);
